...
CahootsSync.prototype.run = function run () {
    ...
    function onFetchPersons (err, fetched) {
        ...
        debug('-> 2. fetched all persons');

        persons.push.apply(persons, fetched);

        debug('3. fetch all cahoots');
    ...
    function onFetchCahoots (err, fetched) {
        ...
        debug('-> 3. fetched all cahoots');

        cahoots.push.apply(cahoots, fetched);

        debug('4. Map the cahoots to the persons');
        ...
        debug('-> 4. Mapped the cahoots into the persons');

        debug('5. wipe the storage');
    ...
    function onDestroy (err) {
        ...
        debug('-> 5. wiped the storage');

        debug('6. store all `organizations`');
    ...
    function onStoreOrganizations (err) {
        ...
        debug('-> 6. stored all `organizations`');
